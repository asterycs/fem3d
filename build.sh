...
mkdir $SDL2_BUILD_PREFIX
cd $SDL2_BUILD_PREFIX
cmake -DCMAKE_BUILD_TYPE=Release -DWITH_OPENGEXIMPORTER=ON -DWITH_DDSIMPORTER=ON -DWITH_STBIMAGECONVERTER=ON -DWITH_STBIMAGEIMPORTER=ON -DWITH_STBTRUETYPEFONT=ON -DWITH_HARFBUZZFONT=ON ..
...
if [ "$BUILD_EMSCRIPTEN" = true ]; then
  ...
  mkdir $EMSCRIPTEN_BUILD_PREFIX
  cd $EMSCRIPTEN_BUILD_PREFIX
  cmake -DCMAKE_BUILD_TYPE=Release -DWITH_OPENGEXIMPORTER=ON -DWITH_DDSIMPORTER=ON -DWITH_STBIMAGECONVERTER=ON -DWITH_STBIMAGEIMPORTER=ON -DWITH_STBTRUETYPEFONT=ON -DWITH_HARFBUZZFONT=ON -DBUILD_PLUGINS_STATIC=ON -DCMAKE_TOOLCHAIN_FILE="../toolchains/generic/Emscripten-wasm.cmake" -DCMAKE_INSTALL_PREFIX=$EMSCRIPTEN_PREFIX ..
  make -j
  make install
